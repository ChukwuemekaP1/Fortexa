          ...
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-[hsl(var(--muted-foreground))]">
                ...
                <span className="sm:hidden">{new Date(entry.timestamp).toLocaleDateString()} {new Date(entry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
          ...
          {!compact ? (
            <div className="mt-2 grid gap-2 sm:grid-cols-3">
              <div className="rounded-lg bg-[hsl(var(--muted)/0.35)] px-3 py-2 text-xs">
                ...
                <p className="truncate" title={entry.action.name}>{entry.action.name}</p>
              </div>
              <div className="rounded-lg bg-[hsl(var(--muted)/0.35)] px-3 py-2 text-xs">
